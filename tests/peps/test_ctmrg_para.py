...
def test_ctmrg_Ising(config_kwargs):
    ...
    for info in PARActmrg_(env, opts_svd_ctm=opts_svd, max_sweeps=200, corner_tol=1e-12, n_cores=4, ctm_jobs_vh=[[psi.sites()], [psi.sites()]]):
        pass
...
#         for info in PARActmrg_(env, opts_svd_ctm=opts_svd, max_sweeps=100, corner_tol=1e-5, n_cores=4, ctm_jobs_vh=[[psi.sites()], [psi.sites()]]):
#             pass
...
def test_ctmrg_Ising_4x5(config_kwargs):
    ...
    """
    config = yastn.make_config(sym='Z2', **config_kwargs)
    opt = yastn.operators.Spin12(sym='Z2', **config_kwargs)
    ...
    for info in PARActmrg_(env, opts_svd_ctm=opts_svd, max_sweeps=10, corner_tol=1e-12):
        pass
    ...
    for site in env.sites():
        print(site, env[site] is None, env.psi[site] is None)
        for keyword in ['t', 'l', 'b', 'r', 'tl', 'tr', 'bl', 'br']:
            print(getattr(env[site], keyword) is None, end=' ')
        print()
    ...
    for info in PARActmrg_(env, opts_svd_ctm=opts_svd, max_sweeps=10, corner_tol=1e-12, n_cores=8):
        pass
    assert info.converged
    #
    ...
    for info in PARActmrg_(env, opts_svd_ctm=opts_svd, max_sweeps=4, corner_tol=1e-8, n_cores=8):
        pass
    # assert info.converged
...
@pytest.mark.parametrize("checkpoint_move", [False])
def test_ctmrg_Ising_dense(checkpoint_move, config_kwargs):
    ...
    opt = yastn.operators.Spin12(default_dype="complex128", **config_kwargs)
    ...
    for info in PARActmrg_(env=env, opts_svd_ctm=opts_svd, max_sweeps=200, corner_tol=1e-10, n_cores=4):
        pass
...
if __name__ == '__main__':
    pytest.main([__file__, "-vv", "--durations=0", "--backend", "torch"]) #,  "--long_tests", ])
