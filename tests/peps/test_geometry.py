""" Test operation of yaps.Lattice and yaps.Peps that inherits Lattice"""
import pytest
import yaps

def test_Lattice():
    """ Generate a few lattices veryfing expected output of some functions. """
    net = yaps.Lattice(lattice='checkerboard', boundary='infinite')
    assert net.dims == (2, 2)
    ...
    net = yaps.Lattice(lattice='rectangle', dims=(3, 2), boundary='finite')
    assert net.dims == (3, 2)
...
def test_NtuEnv():
    ...
    bd00_01_h = yaps.Bond(site_0 = (0, 0), site_1=(0, 1), dirn='h')
    bd11_21_v = yaps.Bond(site_0 = (1, 1), site_1=(2, 1), dirn='v')

    net_1 = yaps.Lattice(lattice='rectangle', dims=(3, 4), boundary='finite')  # dims = (rows, columns) # finite lattice
    ...
    net_2 = yaps.Lattice(lattice='rectangle', dims=(3, 4), boundary='infinite')  # dims = (rows, columns) # infinite lattice
...
def test_Peps_get_set():
    ...
    net = yaps.Peps(lattice='checkerboard', dims=(2, 2), boundary='infinite')
    ...
    net = yaps.Peps(lattice='rectangle', dims=(3, 3), boundary='finite')
    assert all(net[site] == None for site in net.sites())  # all tensors initialized as None
    net[(0, 0)] = "Wannabe tensor"
