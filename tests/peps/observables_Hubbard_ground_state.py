...
def Hubbard_GS(lattice, boundary, purification, xx, yy, D, sym, chi, interval, beta_start, beta_end, UU, mu_up, mu_dn, t_up, t_dn, step, tr_mode, fix_bd):
    ...
    for beta in beta_range:
        ...
        dens_up, cs_up, mat_up = one_site_avg(tpeps, env1, n_up)
        print('density of down spin')
        dens_dn, cs_dn, mat_dn = one_site_avg(tpeps, env1, n_dn)
        print('double occupancy')
        doc, cs_doc, mat_doc = one_site_avg(tpeps, env1, n_int)
        print('hole')
        hole, cs_hole, mat_hole = one_site_avg(tpeps, env1, h)
        ...
        imb[s,4] = hole # hole density
        imb[s,5] = cdagc_up
        imb[s,6] = ccdag_up
        imb[s,7] = cdagc_dn
        imb[s,8] = ccdag_dn
        imb[s,9] = (xx*yy)*UU*doc - (cdagc_up + ccdag_up + cdagc_dn + ccdag_dn)*(2*xx*yy-xx-yy)
        s = s+1
...
if __name__== '__main__':
    ...
    parser.add_argument("-BETA_END", type=float, default=0.5) # location
