...
def test_block_U1(config_kwargs):
    ...
    H2d = H1.drop_leg_history(axes=0)
    H2d = H2d.transpose(axes=(1, 3, 0, 2)).drop_leg_history(axes=(0, 1, 3))
    H2d = H2d.transpose(axes=(2, 0, 3, 1))
    assert yastn.norm(H2d - H0) < tol


def test_block_exceptions(config_kwargs):
