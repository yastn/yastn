...
def _run_to_dict_nonabelian(config_kwargs, sym):
    config = yastn.make_config(sym=sym, **config_kwargs)
    a = four_leg_tensor(config)
    restored = yastn.from_dict(a.to_dict())
    assert restored.config.sym.SYM_ID == sym
    assert get_blocks(config.sym, restored.struct).channels == get_blocks(config.sym, a.struct).channels
    assert (restored - a).norm() < 1e-12


def test_to_dict_SU2(config_kwargs):
    _run_to_dict_nonabelian(config_kwargs, 'SU2')


def test_to_dict_SU2xU1(config_kwargs):
    _run_to_dict_nonabelian(config_kwargs, 'SU2xU1')


def test_to_dict_explicit_SU2_channel_and_matching(config_kwargs):
    config = yastn.make_config(sym='SU2', **config_kwargs)
    half = yastn.Leg(config, s=1, t=(1,), D=(1,))
    scalar = yastn.Leg(config, s=1, t=(0,), D=(1,))
    a = yastn.rand(config, legs=(half, half, scalar))
    a = a._replace(struct=a.struct.replace(channels=((0,),)))
    restored = yastn.from_dict(a.to_dict())
    assert restored.struct.channels == ((0,),)
    assert (restored - a).norm() < 1e-12
    charges = np.array([[[1], [1]], [[1], [1]]], dtype=np.int64)
    ia, ib = find_matching_block_keys(charges, ((0,), (2,)),
                                      charges[::-1], ((2,), (0,)))
    assert ia.tolist() == [1, 0] and ib.tolist() == [0, 1]
