...
def _run_vdot_nonabelian(config_kwargs, sym):
    config = yastn.make_config(sym=sym, **config_kwargs)
    a = four_leg_tensor(config)
    assert np.isclose(yastn.vdot(a, a).real, a.norm() ** 2)


def test_vdot_SU2(config_kwargs):
    _run_vdot_nonabelian(config_kwargs, 'SU2')


def test_vdot_SU2xU1(config_kwargs):
    _run_vdot_nonabelian(config_kwargs, 'SU2xU1')
...
def test_vdot_fuse_hard(config_kwargs, remove_blocks):
    ...
    a2 = a2.remove_random_blocks(number=remove_blocks, keep_legs=True)
