...
def test_reset0():
    print('----------')
    print('3d tensor:')
    a = tensor.ones(settings=settings_full_R, s=(-1, 1, 1), D=(1, 2, 3))
    a.show_properties()
    npa = a.to_numpy()
    ...
    print('----------')
    print('0d tensor:')
    a = tensor.ones(settings=settings_full_R)  # s=() D=()
    a.show_properties()
    npa = a.to_numpy()
    assert np.isrealobj(npa)
    assert npa.shape == ()
    ...
    print('----------')
    print('1d tensor:')
    a = tensor.zeros(settings=settings_full_R, s=1, D=5)  # s=(1,)
    a.show_properties()
    npa = a.to_numpy()
    assert np.isrealobj(npa)
    assert npa.shape == (5,)
    ...
    a = tensor.rand(settings=settings_full_R, isdiag=True, D=5)
    a.show_properties()
    npa = a.to_numpy()
    assert np.isrealobj(npa)
...
def test_reset1():
    print('----------')
    print('4d tensor: ')
    a = tensor.ones(settings=settings_U1_R, s=(-1, 1, 1, 1),
                    t=((-2, 0, 2), (0, 2), (-2, 0, 2), 0),
                    D=((1, 2, 3), (1, 2), (1, 2, 3), 1))
    a.show_properties()
    npa = a.to_numpy()
    ...
    print('----------------')
    print('diagonal tensor:')
    a = tensor.eye(settings=settings_U1_R, t=(-1, 0, 1), D=(2, 3, 4))
    a.show_properties()
    npa = a.to_numpy()
    assert np.isrealobj(npa)
    assert npa.shape == (9, 9)
...
def test_examples_in_reset_tensor():
    a = tensor.ones(settings=settings_U1_R, s=(-1, 1, 1),
                    t=[0, (-2, 0), (2, 0)],
                    D=[1, (1, 2), (1, 3)])
    a.show_properties()
    ...
    a1 = tensor.ones(settings=settings_U1_R, s=(-1, 1, 1), t=ta, D=da)
