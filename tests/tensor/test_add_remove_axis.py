""" yast.add_axis() yast.remove_axis() """
import pytest
import yast
try:
...
def _test_add_remove_axis(a):
    """ run a sequence of adding and removing axis operations. """
    b = yast.add_axis(a)  # new axis with s=1 added as the last one; makes copy
    assert b.is_consistent()
    assert yast.are_independent(a, b)
    assert all(x == 0 for x in b.struct.n)  # tensor charge is set here to 0
    b.add_axis(axis=1, s=-1, inplace=True)
    b.is_consistent()

    c = b.remove_axis()  # removes last axis by default
    assert c.is_consistent()
    assert c.struct.n == a.struct.n
    c.remove_axis(axis=1, inplace=True)
    assert c.is_consistent()
    assert yast.norm(a - c) < tol


def test_add_axis_basic():
    """ add_axis for various symmetries """
    # dense
    a = yast.rand(config=config_dense, s=(-1, 1, 1, -1), D=(2, 3, 4, 5))
    _test_add_remove_axis(a)
    ...
    _test_add_remove_axis(a)
    _test_add_remove_axis(b)

    ab1 = yast.tensordot(a, b, axes=((), ()))
    a.add_axis(s=1, inplace='True')
    b.add_axis(s=-1, inplace='True')
    ...
    a.add_axis(s=-1, axis=0, t=(0, 2), inplace=True)
    assert a.struct.n == (1, 0)
    a.is_consistent()
    a.add_axis(s=1, t=(1, 0), inplace=True)
    ...
    a.add_axis(axis=1, inplace=True)
    assert a.get_shape() == (27, 1, 25)
    a.add_axis(axis=3, inplace=True)
...
def test_operators_chain():
    ...
    nn = (0,) * len(c.n)
    o4 = yast.add_axis(c, axis=-1, t=nn, s=-1)
    o4 = yast.add_axis(o4, axis=0, s=1)
    tD = o4.get_leg_structure(axis=0)
    nn = next(iter(tD))

    o3 = yast.add_axis(c, axis=-1, t=nn, s=-1)
    o3 = yast.add_axis(o3, axis=0, s=1)
    tD = o3.get_leg_structure(axis=0)
    nn = next(iter(tD))

    o2 = yast.add_axis(cdag, axis=-1, t=nn, s=-1)
    o2 = yast.add_axis(o2, axis=0, s=1)
    tD = o2.get_leg_structure(axis=0)
    nn = next(iter(tD))

    o1 = yast.add_axis(cdag, axis=-1, t=nn, s=-1)
    o1 = yast.add_axis(o1, axis=0, s=1)
    ...
    a.remove_axis(axis=1, inplace=True)
...
def test_add_axis_exceptions():
    """ handling exceptions in yast.add_axis()"""
    with pytest.raises(yast.YastError):
        a = yast.eye(config=config_U1, t=(-1, 0, 1), D=(2, 3, 4))
        a.add_axis(s=1)  # Cannot add axis to a diagonal tensor.
    with pytest.raises(yast.YastError):
        a = yast.ones(config=config_U1, s=(1, -1), t=((-1, 0, 1), (-1, 0, 1)), D=((2, 3, 4), (2, 3, 4)), n=1)
        a.add_axis(s=1, t=(1, 0))  # len(t) does not match the number of symmetry charges.
    with pytest.raises(yast.YastError):
        a = yast.ones(config=config_U1, s=(1, -1), t=((-1, 0, 1), (-1, 0, 1)), D=((2, 3, 4), (2, 3, 4)), n=1)
        a.add_axis(s=2)  # Signature of the new axis should be 1 or -1.


def test_remove_axis_exceptions():
    """ handling exceptions in yast.remove_axis()"""
    t, D = (-1, 0, 1), (2, 3, 4)
    with pytest.raises(yast.YastError):
        a = yast.eye(config=config_U1, t=t, D=D)
        a.remove_axis(axis=1)  # Cannot remove axis to a diagonal tensor.
    with pytest.raises(yast.YastError):
        a = yast.ones(config=config_U1, s=(1, -1), t=(t, t), D=(D, D), n=1)
        scalar = yast.tensordot(a, a, axes=((0, 1), (0, 1)), conj=(0, 1))
        _ = scalar.remove_axis(axis=0)  # Cannot remove axis of a scalar tensor.
    with pytest.raises(yast.YastError):
        a = yast.rand(config=config_U1, s=(1, -1, 1), t=(t, t, t), D=(D, D, D))
        a.fuse_legs(axes=((0, 1), 2), mode='meta', inplace=True)
        _ = a.remove_axis(axis=0)  # Axis to be removed cannot be fused.
    with pytest.raises(yast.YastError):
    ...
    with pytest.raises(yast.YastError):
        a = yast.rand(config=config_U1, s=(1, -1, 1, 1), t=(t, t, t, t), D=(D, D, D, D))
        _ = a.remove_axis(axis=1)  # Axis to be removed must have single charge of dimension one.


if __name__ == '__main__':
    test_add_axis_basic()
    test_operators_chain()
    test_add_axis_exceptions()
    test_remove_axis_exceptions()
