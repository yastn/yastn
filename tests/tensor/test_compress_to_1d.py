...
def run_simple_compress(a):
    ...
    b = yast.decompress_from_1d(r1d, meta=meta)
...
def run_compress_wit_meta(a, a_reference) :
    ...
    c = yast.decompress_from_1d(r1d, meta=meta)
...
def test_compress_to_1d_basic():
    ...
    run_simple_compress(a)
    # diagonal Z2xU1
    leg = yast.Leg(config_Z2xU1, s=1, t=((0, -1), (1, 0), (0, 1)), D=(2, 3, 4))
    a = yast.rand(config=config_Z2xU1, isdiag=True, legs=leg)
    run_simple_compress(a)
...
def test_compress_to_1d_embed():
    ...
    a_reference = yast.zeros(config=a.config, legs=a.get_legs())
...
def test_compress_to_1d_exceptions():
    ...
    ad = yast.eye(config=config_U1, legs=yast.Leg(config_U1, s=1, t=(0, 1), D=(2, 3)))
    ...
    with pytest.raises(yast.YastError):
        b = yast.Tensor(config=config_U1, s=(1, -1))
