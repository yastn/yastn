...
tol_ad = {"float64": 1e-8, "complex128": 1e-8,  #pylint: disable=invalid-name
          "float32": 2e-4, "complex64": 2e-4}  #pylint: disable=invalid-name
...
def _test_tensordot_grad(a, b, axes, dtype):
    ...
    assert (ref_cost_f.item() - cost_f.item()) < tol[dtype]
    tt = tol_ad[dtype]
...
def test_tensordot_fuse_hard_backward_01(config_kwargs,dtype):
    ...
    _test_tensordot_grad(a, b, axes=(1, 0), dtype=dtype)
...
def test_tensordot_fuse_hard_backward_1(config_kwargs,dtype):
    ...
    _test_tensordot_grad(a, b.conj(), axes=(1, 0), dtype=dtype)
...
def test_tensordot_fuse_hard_backward_13(config_kwargs):
    ...
    _test_tensordot_grad(a, b, axes=(1, 1), dtype=dtype)
...
def test_tensordot_fuse_hard_Z2xU1(config_kwargs):
    ...
    _test_tensordot_grad(a,b, axes=((0, 1), (0, 1)), dtype=dtype)
    _test_tensordot_grad(b,a, axes=((1, 0), (1, 0)), dtype=dtype)
