...
# TODO QR decomposition not implemented
...
def svd_combine(a):
    ...
    assert isclose(a.norm_diff(USV), 0, rel_tol=rel_tol, abs_tol= abs_tol)
...
#     assert isclose(a.norm_diff(QR), 0, rel_tol=rel_tol, abs_tol=abs_tol)
...
def eigh_combine(a):
    ...
    assert isclose(a2.norm_diff(USU), 0, rel_tol=rel_tol, abs_tol=abs_tol)
    assert U.is_symmetric()
    assert S.is_symmetric()
