...
def test_transpose_0():
    ...
    b = a.transpose(axes=(1, 3, 2, 0)).to_dense()
    assert b.shape == (3, 5, 4, 2)
    c = a.moveaxis(source=1, destination=-1)
    assert c.get_shape_all() == (2, 4, 5, 3)
...
def test_transpose_1():
    ...
    bs = b.get_shape_all()
    assert b.to_dense().shape == (9, 13, 11, 5, 3, 7)
    assert bs == (9, 13, 11, 5, 3, 7)
    c = a.moveaxis(source=1, destination=4)
    cs = c.get_shape_all()
    assert c.to_dense().shape == (5, 13, 11, 7, 9, 3)
