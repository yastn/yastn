...
def test_ncon_0():
    ...
    assert x.get_total_shape() == (4, 3, 2, 1)
   
    y = yast.ncon([a, b, c, d], [[4, -3, -1], [-4, -2, 5], [4, 3, 1, 1], [3, 2, 5, 2]], [0, 1, 0, 1]) 
    assert y.get_total_shape() == (1, 2, 3, 4)

    z1 = yast.ncon([e, f], [[-3, -1], [-2, -4]], [0, 1]) 
    z2 = yast.ncon([f, e], [[-2, -4], [-3, -1]], [1, 0])     
    z3 = yast.ncon([e, f], [[-3, -1], [-2, -4]], [0, 0]) 
    z4 = yast.ncon([f, e], [[-2, -4], [-3, -1]], [1, 1]) 
    assert z1.get_total_shape() == (1, 2, 3, 4)
    assert z2.get_total_shape() == (1, 2, 3, 4)
    assert z3.get_total_shape() == (1, 2, 3, 4)
    assert z4.get_total_shape() == (1, 2, 3, 4)
    assert pytest.approx(z1.norm_diff(z2)) == 0
    assert pytest.approx((z3-z4.conj()).norm()) == 0

    y1 = yast.ncon([a, b, c, d, g, h], [[4, -3, -1], [-4, -2, 5], [4, 3, 1, 1], [3, 2, 5, 2], [-5], [-6]], [1, 0, 1, 0, 1, 0]) 
    assert y1.get_total_shape() == (1, 2, 3, 4, 5, 6)

    y2 = yast.ncon([a, g, a], [[1, 2, 3], [-1], [1, 2, 3]], [0, 0, 1])
    assert y2.get_total_shape() == (5,)

    y3 = yast.ncon([a, g, a, b, a], [[1, 2, 3], [-5], [1, 2, 3], [-4, -2, 4], [4, -3, -1]], [0, 0, 1, 0, 0])
    assert y3.get_total_shape() == (1, 2, 3, 4, 5)
