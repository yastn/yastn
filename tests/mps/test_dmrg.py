...
def run_dmrg_0site(psi, H, occ, Etarget, occ_target, sweeps=10):
    ...
    assert abs(env.measure() - Etarget) < 1e-4  # This seems to be coverging slowly
    assert abs(mps.measure_mpo(psi, occ, psi) - occ_target) < 1e-4  # This seems to be coverging slowly
...
def run_dmrg_1site(psi, H, occ, Etarget, occ_target, sweeps=5):
    ...
    assert abs(env.measure() - Etarget) < 1e-6
    assert abs(mps.measure_mpo(psi, occ, psi)- occ_target) < 1e-4  # This seems to be coverging slowly
