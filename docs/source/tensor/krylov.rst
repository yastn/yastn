...
They assume a linear operation acting on a generalized vector, where the vector is an instance of a class that provides methods
such as ``norm``, ``vdot``, ``add``, and ``expand_krylov_space``.
