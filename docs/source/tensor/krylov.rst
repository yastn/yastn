...
We provide a high level implementation of some Krylov-based algorithms that are used in :class:`yast.tn.mps`.
They assume a linear operation acting on a generalized vector, with the vector being an instance of a class including methods
``norm``, ``vdot``, ``linear_combination``, ``expand_krylov_space``, among others.
Examples of such vector include :class:`yast.Tensor` (see :ref:`methods<tensor-aux>`).
