...
which is :math:`d^N`. 
Tensor Network representation introduces a concept of efficient separation of variables which will iteratively split `j`-th particle from others by performing :ref:`spectral decompositions<tensor/algebra:Spectral decompositions and truncation>`, e.g., singular value decomposition 
