...
- `Leg` got method `.history()` that return string representation of fusion history, with
  'o' marking original legs, `s` is for sum, `p` is for product, 'm' is for meta fusion.