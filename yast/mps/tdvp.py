from yast import linalg, tensordot
import logging
import numpy as np
...
def tdvp_sweep_1site(psi, H=False, M=False, dt=1., env=None, hermitian=True, k=4, eigs_tol=1e-12, optsK_svd=None, **kwargs):
    r"""
    ...
    for n in psi.g.sweep(to='last'):  # sweep from first to last
        if M:  # apply the Kraus operator
            tmp = tensordot(M.A[n], psi.A[n], axes=(2, 1))
            tmp.swap_gate(axes=(0, 2), inplace=True)  # for fermions
            u, s, _ = linalg.svd(tmp, axes=((2, 1, 4), (0, 3)), **optsK_svd)  # discard V
            psi.A[n] = tensordot(u, s, axes=(3, 0)).transpose(axes=(0, 1, 3, 2))
        # forward in time evolution of a single site: T(+dt/2)
        if H:
            psi.A[n] = linalg.expmv(lambda v: env.Heff1(v, n), psi.A[n], 0.5 * dt, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
        ...
        if H and n != psi.g.sweep(to='last')[-1]:
            psi.A[psi.pC] = linalg.expmv(lambda v: env.Heff0(v, psi.pC), psi.A[psi.pC], -0.5 * dt, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
...
def tdvp_sweep_2site(psi, H=False, M=False, dt=1., env=None, hermitian=True, k=4, eigs_tol=1e-12, opts_svd=None, optsK_svd=None, **kwargs):
    r"""
    ...
    for n in psi.g.sweep(to='last', dl=1):
        ...
        if H:
            ...
            init = linalg.expmv(lambda v: env.Heff2(v, n), v=init, t=+0.5 * dt, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
            # split and save
        ...
        if H and n != psi.g.sweep(to='last', dl=1)[-1]:
            psi.A[n1] = linalg.expmv(lambda v: env.Heff1(v, n1), psi.A[n1], t=-dt * .5, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
    ...
    for n in psi.g.sweep(to='first', df=1):
        ...
        if H:
            ...
            init = linalg.expmv(lambda v: env.Heff2(v, n1), v=init, t=0.5 * dt, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
            # split and save
            A1, S, A2 = linalg.svd(init, axes=(psi.left + psi.phys, tuple(a + psi.right[0] - 1 for a in psi.phys + psi.right)), sU=-1, **opts_svd)
...
def tdvp_sweep_2site_group(psi, H=False, M=False, dt=1, env=None, hermitian=True, fermionic=False, k=4, eigs_tol=1e-12, exp_tol=1e-12, bi_orth=True, NA=None, opts_svd=None, optsK_svd=None, algorithm='arnoldi'):
    ...
    for n in psi.g.sweep(to='last', dl=1):
        ...
        if H:
            ...
            init = linalg.expmv(lambda v: env.Heff2_group(v, n), v=init, t=0.5 * dt, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
            # split and save
    ...
    for n in psi.g.sweep(to='first', df=1):
        ...
        if H:
            ...
            init = psi.A[n1].tensordot(psi.A[n], axes=(psi.right, psi.left))
            init.fuse_legs(axes=(0, (1, 2), 3), inplace=True)
            init = linalg.expmv(lambda v: env.Heff2_group(v, n1), v=init, t=0.5 * dt, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
            # split and save
            init.unfuse_legs(axes=1, inplace=True)
            A1, S, A2 = linalg.svd(init, axes=(psi.left + psi.phys, tuple(a + psi.right[0] - 1 for a in psi.phys + psi.right)), sU=-1, **opts_svd)
            psi.A[n1] = A1.tensordot(S, axes=(psi.right, 0))
...
def tdvp_sweep_mix(psi, SV_min, versions, H=False, M=False, dt=1., env=None, hermitian=True, fermionic=False, k=4, eigs_tol=1e-12, exp_tol=1e-12, bi_orth=True, NA=None, D_totals=None, tol_svds=None, opts_svd=None, optsK_svd=None, algorithm='arnoldi'):
    ...
    for n in psi.g.sweep(to='last'):
        ...
        if version == '1site':
            # matrix exponentiation, forward in time evolution of a single site: T(+dt*.5)
            if H:
                psi.A[n] = linalg.expmv(lambda v: env.Heff1(v, n), psi.A[n], t=0.5 * dt, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
            # canonize and save
            psi.orthogonalize_site(n, towards=psi.g.last)
            env.clear_site(n)
            env.update(n, towards=psi.g.last)
            ...
            if H and n != psi.g.sweep(to='last')[-1]:
                psi.A[psi.pC] = linalg.expmv(lambda v: env.Heff0(v, psi.pC), psi.A[psi.pC], t=-dt * .5, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
            psi.absorb_central(towards=psi.g.last)
        elif version == '2site':
            ...
            if n == psi.g.sweep(to='last')[-1]:
                ...
                # matrix exponentiation, backward in time evolution of a single site: T(-dt*.5)
                if H and n != psi.g.sweep(to='last', dl=1)[-1]:
                    psi.A = linalg.expmv(lambda v: env.Heff1(v, n1), psi.A[n1], t=-dt * .5, tol=eigs_tol, ncv=k, hermitian=hermitian, normalize=True)
    Ds = psi.get_D()
    for n in psi.g.sweep(to='first'):
        opts_svd['D_total'] = D_totals[n]
        opts_svd['tol'] = tol_svds[n]
