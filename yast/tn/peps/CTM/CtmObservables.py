...
def nn_avg(fid, Gamma, net, env, op):
    ...
    for ms in op.keys():
        ...
        for bds_h in net.bonds(dirn='h'):  # correlators on all horizontal bonds
            ...
            AAbo = ret_AAbs(fid, Gamma, bds_h, opt, orient='h')
            AAb = {'l': fPEPS_2layers(fid, Gamma._data[bds_h.site_0]), 'r': fPEPS_2layers(fid, Gamma._data[bds_h.site_1])}
        ...
        for bds_v in net.bonds(dirn='v'): # correlators on all vertical bonds
            AAbo = ret_AAbs(fid, Gamma, bds_v, opt, orient='v')
            AAb = {'l': fPEPS_2layers(fid, Gamma._data[bds_v.site_0]), 'r': fPEPS_2layers(fid, Gamma._data[bds_v.site_1])}
...
def nn_bond(fid, Gamma, net, env, op, bd):
    # calculation of correlator on a desired bond in the lattice
    # useful when you don't need the average value

    Gamma = check_consistency_tensors(Gamma, net)
    AAbo = ret_AAbs(fid, Gamma, bd, op, orient=bd.dirn)
    AAb = {'l': fPEPS_2layers(fid, Gamma._data[bd.site_0]), 'r': fPEPS_2layers(fid, Gamma._data[bd.site_1])}
