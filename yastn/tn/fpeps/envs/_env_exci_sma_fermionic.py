...
class EnvExciSMAFermionic(EnvExciSMA):
    """
    Fermionic variant of ``EnvExciSMA``.

    The base implementation already handles fermionic sign strings in
    ``measure_exci``. The extra care needed here is preserving the internal
    ``DoublePepsTensor`` metadata (local operator plus accumulated charge-swap
    strings) whenever we temporarily replace bra/ket layers with excitation
    tensors during auxiliary sweeps.
    """

    @staticmethod
    def _replace_layers(template, bra=None, ket=None):
        return DoublePepsTensor(
...
EnvExciSMA = EnvExciSMAFermionic
