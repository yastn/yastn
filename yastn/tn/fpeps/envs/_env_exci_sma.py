...
class EnvExciSMA:
    ...
    def __getitem__(self, ind) -> mps.MpsMpoOBC:
        ...
        if dirn == 'hb':
            ...
            for ind, ny in enumerate(range(*self.yrange), start=1):
                ...
                new_ket = exci_ket if (n, ny) == site_ket else self.psi[n, ny].ket
                new_psi = DoublePepsTensor(bra=new_bra, ket=new_ket)
                op.A[ind] = new_psi.transpose(axes=(1, 0, 3, 2))
            op.A[0] = self.env_ctm[n, self.yrange[0]].l.add_leg(axis=0).transpose(axes=(0, 3, 2, 1))
            op.A[self.Ny + 1] = self.env_ctm[n, self.yrange[1] - 1].r.add_leg(axis=3).transpose(axes=(1, 0, 3, 2))
            return op
    ...
    def measure_exci_ops(self, *operators, exci_psi=None, sites_op=None, opts_svd=None, opts_var=None):
        ...
        ny0 = sites[0][1]
        ny1 = sites[-1][1]
        out = {tuple(site): None for site in sites} # init in given order
        nxc, nyc = (nx0 + nx1) // 2, (ny0 + ny1) // 2 # center

        # Construct all the tms
        # tms = {nx: self[nx, 'h'] for nx in range(*self.xrange)}
        tms = {}
        tmbs = {}
        for nx in range(*self.xrange):
        ...
        # Construct all the top vecs or bottom veccs that could include operators
        ...
        # all top vectors that include all the operators
        for nx in range(nx_op_min+1, self.xrange[1]):
            vecs[nx] = mps.zipper(tms[nx-1], vecs[nx-1], opts_svd=opts_svd, normalize=False)
            mps.compression_(vecs[nx], (tms[nx-1], vecs[nx-1]), method='1site', normalize=False, **opts_var)

        # all the bottom vectors that include all the operators
        for nx in range(nx_op_max-1, self.xrange[0]-1, -1):
            veccs[nx] = mps.zipper(tmbs[nx+1], veccs[nx+1], opts_svd=opts_svd, normalize=False)
            mps.compression_(veccs[nx], (tmbs[nx+1], veccs[nx+1]), method='1site', normalize=False, **opts_var)
        out = {}
    ...
    def measure_exci_norm(self, exci_bra=None, exci_ket=None, opts_svd=None, opts_var=None):
        ...
        for iy0, ny0 in enumerate(range(*self.yrange), start=1):
            ...
            vecc, tm, tmb, vec = self[nxc, 'b'].conj(), self[nxc, 'h', exci_bra, None, (nxc, nyc), None], self[nxc, 'hb', exci_bra, None, (nxc, nyc), None], self[nxc, 't']
            ...
            if nxc < self.xrange[1] - 1:
                ...
                mps.compression_(vecc_o0_last, (tmb, vecc.conj()), method='1site', normalize=False, **opts_var)  

            ket0 = tm[iy0].ket
            tm[iy0] = DoublePepsTensor(bra=tm[iy0].bra, ket=exci_ket[nxc, ny0]).transpose(axes=(1, 2, 3, 0))
            env.update_env_(iy0, to='first')
            out[(nxc, ny0)] = env.measure(bd=(iy0-1, iy0))

            tm[iy0] = DoublePepsTensor(bra=tm[iy0].bra, ket=ket0).transpose(axes=(1, 2, 3, 0))

            for iy1, ny1 in enumerate(range(ny0 + 1, self.yrange[1]), start=ny0 - self.yrange[0] + 2):
                ket0 = tm[iy1].ket
                tm[iy1] = DoublePepsTensor(bra=tm[iy1].bra, ket=exci_ket[nxc, ny1]).transpose(axes=(1, 2, 3, 0))
                env.update_env_(iy1, to='first')
            ...
            for nx1 in range(nxc-1, self.xrange[0]-1, -1):
                ...
                if nx1 > self.xrange[0]:
                    ...
                    mps.compression_(vecc_o0_last, (tmb, vecc_o0), method='1site', normalize=False, **opts_var)
                ...
                # env = mps.Env(vec, [tmb, vecc_o0.conj()]).setup_(to='last').setup_(to='first')
                # for iy1, ny1 in enumerate(range(*self.yrange), start=1):
                #     ket0 = tmb[iy1].ket
                #     tmb[iy1] = DoublePepsTensor(bra=tmb[iy1].bra, ket=exci_ket[nx1, ny1]).transpose(axes=(1, 0, 3, 2))
                #     env.update_env_(iy1, to='first')
                #     out[(nxc, nyc), (nx1, ny1)] = env.measure(bd=(iy1-1, iy1))
                #     tmb[iy1] = DoublePepsTensor(bra=tmb[iy1].bra, ket=ket0).transpose(axes=(1, 0, 3, 2))
    ...
    def measure_exci_norm_tl(self, exci_bra=None, exci_ket=None, opts_svd=None, opts_var=None):
        ...
        for iy0, ny0 in enumerate(range(*self.yrange), start=1):
            if iy0 > 1:
                break
            vecc, tm, vec = self[nx0, 'b'].conj(), self[nx0, 'h', exci_bra, None, (nx0, ny0), None], self[nx0, 't']
