...
def _validate_matching_charge_sector(A_bra, A_ket):
    """
    Charged excitation contractions require bra/ket tensors to live in the
    same local charge sector. In contrast to the neutral path, the charge does
    not need to be zero.
    """
    if A_ket.config.fermionic and A_bra.n != A_ket.n:
        raise YastnError(
            "Charged auxiliary contractions expect bra and ket tensors in the same charge sector."
        )
...
def append_vec_tl(Ac, A, vectl, op=None, mode='old', in_b=(2, 1), out_a=(2, 3)):
    ...
    _validate_matching_charge_sector(Ac, A)
    if op is not None:
...
def append_vec_tr(Ac, A, vectr, op=None, mode='old', in_b=(1, 2), out_a=(1, 2)):
    ...
    if op is not None:
        A = tensordot(A, op, axes=(4, 1))
    n_vec = A.config.sym.add_charges(vectr.n, A.n)
    # We have to assume that vactr can carry explicit charge;
    # We will swap with it for good swap placement. For charged impurity
    # tensors, the swapped object carries both the boundary-vector charge and
    # the tensor charge (plus the operator charge already folded into A).
...
def append_vec_bl(Ac, A, vecbl, op=None, mode='old', in_b=(2, 1), out_a=(0, 3)):
    ...
    if op is not None:
        A = tensordot(A, op, axes=(4, 1))
    n_vec = A.config.sym.add_charges(vecbl.n, A.n)
