...
def _update_fpctm_projectors(env: EnvCTM, seq='tlbr', opts:dict = None) -> EnvCTM: #"Lattice[EnvCTM_projectors]":
    ...
    biorth_mv= {'l': biorth_left, 'r': biorth_right, 't': biorth_top, 'b': biorth_bottom}
    for mv in seq:   
        for site in env.sites():
            r,c= site
            biorth_mv[mv](r,c)
    # NOTE switch loop over mv and site ?
...
def _update_fpctm_env_T(env: EnvCTM, seq='tlbr', opts:dict= None) -> EnvCTM:
    ...
    # T-tensors
    fpctm_t_mv= {'l': fpctm_t_left, 'r': fpctm_t_right, 't': fpctm_t_top, 'b': fpctm_t_bottom}
    for mv in seq:
        for site in env.sites():
            r,c= site
            fpctm_t_mv[mv](r,c)
