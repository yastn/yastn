...
def tdvp_(psi, H,
          ...
          shift_E=False, precompute=False,
    ...
    if method == '1site':
        routine = lambda t, dt0, env: _tdvp_sweep_1site_(psi, Ht(t), dt0, u, et(env), opts_expmv, normalize, shift_E, precompute)
    elif method == '2site':
        routine = lambda t, dt0, env: _tdvp_sweep_2site_(psi, Ht(t), dt0, u, et(env), opts_expmv, opts_svd, normalize, shift_E, precompute)
    elif method == '12site':
        routine = lambda t, dt0, env: _tdvp_sweep_12site_(psi, Ht(t), dt0, u, et(env), opts_expmv, opts_svd, normalize, shift_E, precompute)
    else:
...
def _tdvp_sweep_1site_(psi, H, dt=0.1, u=1j, env=None, opts_expmv=None, normalize=True, shift_E=False, precompute=False):
    ...
    for to in ('last', 'first'):
        for n in psi.sweep(to=to):
            _update_A(env, n, -u * 0.5 * dt, opts, normalize=normalize, shift_E=shift_E, precompute=precompute)
            psi.orthogonalize_site_(n, to=to, normalize=normalize)
            env.clear_site_(n)
            env.update_env_(n, to=to)
            _update_C(env, u * 0.5 * dt, opts, normalize=normalize, shift_E=shift_E)
...
def _tdvp_sweep_2site_(psi, H, dt=0.1, u=1j, env=None, opts_expmv=None, opts_svd=None, normalize=True, shift_E=False, precompute=False):
    ...
    for to, dn in (('last', 1), ('first', 0)):
        for n in psi.sweep(to=to, dl=1):
            _update_AA(env, (n, n + 1), -u * 0.5 * dt, opts, opts_svd, normalize=normalize, shift_E=shift_E, precompute=precompute)
            psi.absorb_central_(to=to)
            env.clear_site_(n, n + 1)
            env.update_env_(n + 1 - dn, to=to)
            if n + dn != getattr(psi, to):
                _update_A(env, n + dn, u * 0.5 * dt, opts, normalize=normalize, shift_E=shift_E, precompute=precompute)
...
def _tdvp_sweep_12site_(psi, H, dt=0.1, u=1j, env=None, opts_expmv=None, opts_svd=None, normalize=True, shift_E=False, precompute=False):
    r"""
    ...
    for to, dn in (('last', 1), ('first', 0)):
        ...
        for n in psi.sweep(to=to):
            if not update_two:
                ...
                _update_AA(env, (n - dn , n - dn + 1), - u * 0.5 * dt, opts, opts_svd, normalize=normalize, shift_E=shift_E, precompute=precompute)
                psi.absorb_central_(to=to)
...
def _update_A(env, n, du, opts, normalize=True, shift_E=False, precompute=False):
...
def _update_C(env, du, opts, normalize=True, shift_E=False):
...
def _update_AA(env, bd, du, opts, opts_svd, normalize=True, shift_E=False, precompute=False):
