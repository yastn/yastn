...
def _nonzero_sectors(backend, data, sym, struct):
    """
    Return per-block flags for a merged matrix.

    Each value is ``True`` if the corresponding block contains any nonzero element,
    and the order matches ``get_blocks(sym, struct)``. Returns ``None`` when all
    blocks are nonzero and no filtering is needed; for an all-zero matrix every
    sector is dropped, giving empty ``U``, ``S``, and ``V``.
    """
