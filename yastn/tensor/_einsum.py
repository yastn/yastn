...
def _execute_commands(ts, commands, release_cuda_cache=False):
    for command in commands:
        if command[0] in ('tensordot', 'tensordot_psplit'):
            ...
            ts[tout] = _contract_psplit(partial(tensordot, b=b, axes=axes), a, paxes[0]) if paxes \
                else tensordot(a, b, axes=axes)
            ...
            a = ts.pop(tin)
            ts[tout] = _contract_psplit(partial(trace, axes=axes), a, paxes[0]) if paxes else trace(a, axes=axes)
