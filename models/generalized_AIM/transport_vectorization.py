...
def vectorized_Lindbladian(tensor_type, LSR, temp, dV, gamma, corr, basis, AdagA=False):
    if basis == 'Dirac':
        return vectorized_Lindbladian_general(tensor_type, LSR, temp, dV, gamma, corr, basis, AdagA)
...
def vectorized_Lindbladian_general(tensor_type, LSR, temp, dV, gamma, corr, basis, AdagA=False):
    ...
    for n in range(N):
        ...
        if abs(LSR[n]) == 1:
            ...
            diss_off = 0.*II
        if n == 0:
            H.A[n] = tensor.block({(0, 0): On_Site + diss_off, (0, 1): Z_Q_Z, (0, 2): II}, common_legs=(1, 2))
        elif n == N - 1:
    ...
    for mS in np.nonzero(LSR==2)[0]:
        for n in range(N):
            ...
            H.A[n] = tensor.block({(0, 0): H.A[n], (1, 1): tmp}, common_legs=(1, 2))
...
def vectorized_Lindbladian_real(tensor_type, LSR, temp, dV, gamma, corr, basis, AdagA=False):
    ...
    for mS in np.nonzero(LSR==2)[0]:
        for n in range(N):
            v = corr[mS, n]
            if n == 0:
                tmp = tensor.block({(0, 0): II*0., (0, 1): v*XR_right, (0, 2): v*XI_right, (0, 3): v*YR_right,  (0, 4): v*YI_right, (0, 5): II}, common_legs=(1, 2))
            elif n != 0 and n < mS:
                tmp = tensor.block({(-5, 0): II,
                                    (-4, 1): ZR, (-4, 2): ZI,
                                    (-3, 1): (-1)*ZI, (-3, 2): ZR,
            ...
            if type(ival) is dict:
                ...
                for inm2, ival2 in ival.items():
                    try:
                        ...
                        pass
                    try:
                        big_file.create_dataset(direction_sb+str(it2)+'/'+'block', data=inm2)
                        big_file.create_dataset(direction_sb+str(it2)+'/'+'mat', data=ival2)
                    except:
