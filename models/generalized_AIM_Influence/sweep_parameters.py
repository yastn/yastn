...
from AIM_transport.transport_vectorization import transport_vectorization as main_fun
from AIM_transport.transport_vectorization import transport_vectorization_general as general
import AIM_transport.transport_vectorization_full as main_full
import AIM_transport.transport_vectorization_Z2 as main_Z2
import AIM_transport.transport_vectorization_U1 as main_U1
...
def transport(main, basis, tensor, filename):  # PRA MODEL
    ...
    values = [[Tscale, NS, Tscale], wj, vj, HS, muj, dVj, gamma_val, tempj, discretization,
...
def transport_full(choice):
    transport(main_full, choice, 'full', 'DIAM_full_'+choice)


def transport_Z2(choice):
    if choice == 'Majorana':
        transport(main_Z2, choice, 'Z2', 'DIAM_Z2_'+choice)
    elif choice == 'Dirac':
...
def transport_U1(choice):
    if choice == 'Dirac':
        transport(main_U1, choice, 'U1', 'DIAM_U1_'+choice)
